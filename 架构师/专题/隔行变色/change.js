...
for (let i = 0; i < itemList.length; i++) {
  ...
  itemList[i].onmouseout = function () {
    ...
    this.className = removeClass(this.className, 'hover');
  };

}

function removeClass(classList, targetClass) {
  let tmpArr = classList.split(' ');
  if (tmpArr.includes(targetClass)) {
    tmpArr.splice(tmpArr.indexOf(targetClass), 1);
  }
  return tmpArr.join('');
}
